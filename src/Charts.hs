...
-- I was trying to figure out how to apply the chart stuff from the stockquotes example to the 
-- covid assignment.
...
--              -> FilePath
--              -> IO ()
-- plotChart title covidstats fname = do
...
--       [ (Candle statDate covidCases icuCases,
--         (statDate, icuCases),
--         (statDate, covidCases)) | CovidData {..} <- toList covidstats ]
...
--     candlesLayout =
--        layout_title .~ title
--      $ layout_plots .~ [ toPlot $ qline "INFO" closings green,
--                          toPlot $ candle "Candle" candles cyan ]
--      $ def

--     volumesLayout =
--        layout_plots .~ [ plotBars $ bars "TESTING" volumes gray ]
--      $ def
